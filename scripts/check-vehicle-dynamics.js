...
const base = {
  ...
  accelerationFactor: 1, launchGrip: 1, steerInput: 1, deltaSeconds: 1 / 60
};
const straight = { cornerSpeedKph: Infinity, edgeRatio: 0 };
const tightCorner = { cornerSpeedKph: 95, edgeRatio: 0 };
const simulate = (context, deltaSeconds, onGrass = false, durationSeconds = 20, overrides = {}) => {
  let speed = 0;
  for (let time = 0; time < durationSeconds; time += deltaSeconds) {
    speed = advanceVehicleDynamics({ ...base, ...overrides, speedKph: speed, trackContext: context, onGrass, deltaSeconds }).speedKph;
  }
  return speed;
};
